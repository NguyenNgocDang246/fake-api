...
- `src/models/` — DTOs and Zod schemas, one domain per entry: a file for `auth`, `user`, `project`, `endpoint_group`, `mail` and `api_response`, and a folder for `endpoint/` (whose `scenario.model.ts` owns everything about one saved response, while `base.model.ts` keeps only what addresses an endpoint) and `endpoint_plan/`, each of which re-exports its parts from the file the folder is named after so a caller still imports one path. The blueprint DSL in `endpoint_plan/` has its own guide: [src/models/endpoint_plan/AGENTS.md](src/models/endpoint_plan/AGENTS.md).
