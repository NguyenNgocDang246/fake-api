...
export interface ResendEmailFormHandles {
  submit: () => Promise<boolean>;
}

export const ResendEmailForm = forwardRef<ResendEmailFormHandles>((props, ref) => {
  void props;
  const {
    register,
    handleSubmit,
  ...
  const ResendEmailMutation = useMutation<ApiSuccessResponse, ApiErrorResponse, GetUserByEmailDTO>({
    mutationFn: (data) => api.post(API_ROUTES.AUTH.EMAIL.RESEND, data),
    onSuccess() {
      reset();
  ...
  const onSubmit = async (data: GetUserByEmailDTO): Promise<boolean> => {
    try {
      await ResendEmailMutation.mutateAsync(data);
      return true;
    } catch (error) {
...
ResendEmailForm.displayName = "ResendEmailForm";
