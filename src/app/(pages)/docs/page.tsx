...
export default async function DocsPage() {
        ...
        <div className="max-w-3xl flex flex-col gap-16">
          <section id="introduction" className="scroll-mt-24">
            ...
            <p className="text-gray-600 leading-relaxed mb-4">
              ...
              <code className="text-sm bg-gray-100 rounded px-1.5 py-0.5">projectId</code>, and that
              id is the project&apos;s own subdomain. You define the path, JSON body, HTTP status
              code, and delay for each endpoint, and decide whether that body is served exactly as
              written or regenerated by AI on every call. The final URL format is:
            </p>
            <CodeBlock lang="url">
              {mockEndpointUrl("{projectId}", "{your_created_path}")}
            </CodeBlock>
          </section>
  ...
  "ai_prompt": "keep the email domain example.com"
          ...
          <section id="call-api" className="scroll-mt-24">
            ...
            <div className="flex flex-col gap-4 mb-6">
              <CodeBlock lang="bash">{`curl -X GET ${mockEndpointUrl("k7mqx4rb9dtz", "/api/user/1")}`}</CodeBlock>
              <CodeBlock lang="fetch">{`fetch('${mockEndpointUrl("k7mqx4rb9dtz", "/api/users/1")}')
  .then(response => response.json())
  .then(data => console.log(data))
  .catch(error => console.error('Error:', error));`}</CodeBlock>
              <CodeBlock lang="axios">{`axios.get('${mockEndpointUrl("k7mqx4rb9dtz", "/api/users/1")}')
  ...
  .catch(error => console.error('Error:', error));`}</CodeBlock>
          ...
          <section id="headers-cors" className="scroll-mt-24">
            ...
            <div className="flex flex-col gap-4 mb-6">
              <CodeBlock lang="fetch">{`const response = await fetch('${mockEndpointUrl("k7mqx4rb9dtz", "/api/users")}');
