...
- `constants.ts` — token expiration constants (access/refresh/reset-password/verify-email, both as seconds and string durations for `jose`), and the enums `ERROR_MESSAGES`, `SUCCESS_MESSAGES`, `RESPONSE_STATUS`, `STATUS_CODE`, `AUTH_MESSAGES`, `GOOGLE_AUTH_MESSAGES`, `ENDPOINT_MESSAGES`, `LIMIT_MESSAGES`, `AI_MESSAGES`, `TOKEN_MESSAGE`. All user-facing strings and HTTP status codes live here, nowhere else. It also holds the AI tuning constants (variant pool sizing and refill lock, prompt context caps, output token budget and thinking reserve, rate limit defaults, request and failover timeouts); what each one trades off is documented where it is used, in [../services/ai/AGENTS.md](../services/ai/AGENTS.md) and [../services/endpoint/AGENTS.md](../services/endpoint/AGENTS.md).
