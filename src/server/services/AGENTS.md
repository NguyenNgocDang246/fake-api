...
- `guest.service.ts` + `guest.constants.ts` — the whole guest concept. One shared account, found or lazily created by the fixed `GUEST_EMAIL` and cached on the instance; `createSandbox` gives each visitor their own project under it and returns `{project_id, endpoint_group_id}`. It calls `projectService.createProject` rather than writing the rows itself, and deliberately creates no group: that method already makes one named `"default"`, and `GUEST` is allowed exactly one group per project, so the group is read back instead. `deleteExpiredSandboxes` sweeps by `projects.created_at`, which is why an interrupted `createSandbox` leaving a guest with no project is harmless. `createSandbox` sweeps *before* it asks `canCreateProject`, because most of what fills `ROLE_LIMITS.GUEST.maxProjects` is sandboxes nobody came back to; a visitor arriving when the ceiling is genuinely reached is refused with `TRIAL_FULL` and a 503. Nothing here mints a token, because a visitor has no session at all: the guest identity is attached by [../middlewares/guest.middleware.ts](../middlewares/guest.middleware.ts). `guest.constants.ts` holds literals and enums only, since `src/middleware.ts` imports it.
- `ai_usage.service.ts` — the append-only record of model calls a user has spent, which the per-role daily quota counts. `trySpend` is the one a caller about to reach a model uses: it counts and records inside a single transaction behind a row lock on the user, so concurrent designs cannot all pass the same check. `record` is the bare insert, and `isAiAllowed` answers the different question of whether the role has any AI at all, which is what the endpoint routes refuse `ai_enabled` on. `quotaFor` is the read behind the quota badge in the AI card: it reports `{limit, spent}` with a plain `count`, deliberately without the lock or the transaction, since nothing is being claimed and `trySpend` remains the only thing that decides. The AI preview route returns it with every answer, a free reroll and a quota refusal included, so the badge moves with the response instead of a refetch behind it. It sits here rather than in `ai/` because that directory may not import anything from the domain and this file knows about users and roles.
...
- **Default list ordering**: every `findMany` that lists rows for a client orders by `{ updated_at: "desc" }` (most recently updated first). `endpoint/scenario.service.ts` is the one exception and says why in its own guide. `updated_at` is a Prisma `@updatedAt` column on all four models, auto-set on create/update.
