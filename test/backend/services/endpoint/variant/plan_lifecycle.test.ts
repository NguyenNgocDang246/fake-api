...
describe("loadPlan / isPlanStale", () => {
  it("returns the stored blueprint when the hash still matches", () => {
    expect(endpointVariantPlanService.loadPlan(scenario())).toMatchObject({ version: 1 });
    expect(endpointVariantPlanService.isPlanStale(scenario())).toBe(false);
  });

  it("treats a changed body, selection or hint as stale", () => {
    expect(
      endpointVariantPlanService.isPlanStale(scenario({ response_body: '{"name":"B","age":1}' }))
    ).toBe(true);
    expect(endpointVariantPlanService.isPlanStale(scenario({ ai_fields: ["name"] }))).toBe(true);
    expect(endpointVariantPlanService.isPlanStale(scenario({ ai_prompt: "uuid" }))).toBe(true);
  });

  it("does not treat a reordered selection as stale", () => {
    expect(endpointVariantPlanService.isPlanStale(scenario({ ai_fields: ["age", "name"] }))).toBe(
      false
    );
  });

  it("treats a missing, unparseable or malformed blueprint as stale", () => {
    expect(endpointVariantPlanService.isPlanStale(scenario({ ai_plan: null }))).toBe(true);
    expect(endpointVariantPlanService.isPlanStale(scenario({ ai_plan: "{not json" }))).toBe(true);
    expect(
      endpointVariantPlanService.isPlanStale(
        scenario({ ai_plan: JSON.stringify({ version: 1, fields: [{ path: "a", recipe: {} }] }) })
      )
    ).toBe(true);
  });

  it("reports no blueprint when AI is off or nothing is selected", () => {
    expect(endpointVariantPlanService.loadPlan(scenario({ ai_enabled: false }))).toBeNull();
    expect(endpointVariantPlanService.loadPlan(scenario({ ai_fields: [] }))).toBeNull();
  });
});
...
describe("wouldDesign", () => {
  ...
  it("says no while the stored blueprint still describes the inputs", () => {
    expect(endpointVariantPlanService.wouldDesign(scenario(), null, null, ORIGIN)).toBe(false);
  });
  ...
  it("says no when the edit cannot touch the blueprint, even though the hash moved", () => {
    const edited = scenario({ response_body: '{"name":"An","age":3,"items":[{"price":2}]}' });
  ...
  it("says yes when the edit breaks the blueprint", () => {
    ...
    const broken = scenario({ response_body: '{"name":"An","age":"three","items":[{"price":1}]}' });
  ...
  it("says yes when a ticked field leaves the body altogether", () => {
    const gone = scenario({ response_body: '{"name":"An","items":[{"price":1}]}' });
  ...
  it("says yes when a field is ticked, which no stored blueprint can cover", () => {
    const ticked = scenario({ ai_fields: [...FIELDS, "items[].price"] });
  ...
  it("says yes when a field is unticked", () => {
    expect(
      endpointVariantPlanService.wouldDesign(scenario({ ai_fields: ["name"] }), null, null, ORIGIN)
    ).toBe(true);
  });
  ...
  it("says yes when the hint changes", () => {
    const hinted = scenario({ ai_prompt: "use a uuid for id" });
  ...
  it("does not count a reordered selection or a respaced hint as a change", () => {
    const same = scenario({ ai_fields: ["age", "name"], ai_prompt: "  " });
  ...
  it("says yes for a create, which stores no blueprint yet", () => {
    expect(
      endpointVariantPlanService.wouldDesign(
        scenario({ ai_plan: null, ai_plan_hash: null }),
        null,
        null,
        null
      )
  ...
  it("says no when the blueprint travelling with the save can be adopted", () => {
    const fresh = scenario({ ai_plan: null, ai_plan_hash: null });
  ...
  it("says yes when that blueprint no longer fits the body", () => {
    ...
    const fresh = scenario({ response_body: body, ai_plan: null, ai_plan_hash: null });
  ...
  it("says yes for a stored blueprint that will not parse", () => {
    const broken = scenario({ ai_plan: "{not json", ai_plan_hash: "moved" });
  ...
  it("says no when AI is off or nothing is selected", () => {
    ...
    expect(
      endpointVariantPlanService.wouldDesign(
        scenario({ ...edited, ai_enabled: false }),
        null,
        null,
        ORIGIN
      )
...
describe("carryPlanForward", () => {
  ...
  it("re-stores the blueprint under the hash of the body it survived", async () => {
    ...
    const edited = scenario({ response_body: body });
  ...
  it("refuses when the edit broke the blueprint, leaving the row for a redesign", async () => {
    ...
    expect(prisma.endpoint_scenarios.update).not.toHaveBeenCalled();
  });
  ...
  it("refuses when there is no blueprint to carry", async () => {
    await expect(
      endpointVariantPlanService.carryPlanForward(scenario({ ai_plan: null }), ORIGIN)
    ).resolves.toBe(false);
  });
});
...
describe("planInfoOf", () => {
  ...
  it("defaults a blueprint stored without the field instead of refusing it", () => {
    expect(endpointVariantPlanService.planInfoOf(scenario())).toEqual({
  ...
  it("reports the language the model could not serve", () => {
    ...
    expect(endpointVariantPlanService.planInfoOf(scenario({ ai_plan: flagged }))).toEqual({
  ...
  it("reports nothing once the blueprint has gone stale", () => {
    ...
    expect(
      endpointVariantPlanService.planInfoOf(
        scenario({ ai_plan: flagged, ai_plan_hash: "no-longer-matching" })
      )
    ).toBeUndefined();
  });
});
